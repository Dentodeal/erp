        ...
        <q-dialog v-model="createDialog" persistent >
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div v-if="edit_mode">Edit product brand: {{name}}</div>
                    <div v-else>Create new product brand</div>
                    <q-space />
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        { label: 'Product Brands', link: '/product_brands' }
      ],
      pagePreferences: {
        page_index_visible_columns: 'product_brands_index_visible_columns',
        page_index_pagination: 'product_brands_index_pagination'
      }
    }
  },
  mounted () {
    this.$store.commit('setPageTitle', 'Product Brands')
  },
  computed: {
    toolbarButtons () {
      const arr = []
  ...
  methods: {
    ...
    saveFn () {
      let route = 'product_brands'
      if (this.edit_mode) {
        route = 'product_brands/' + this.edit_id
      }
      const obj = {
