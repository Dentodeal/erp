      ...
      <q-card>
        <q-card-section>
          <div class="row q-col-gutter-md">
            <div class="col-md-4 col-12 q-mt-md">
              <q-input
                dense
                filled
                square
            ...
            <div class="col-md-2 col-12 q-mt-md">
              <q-select
    ...
    breadcrumbs () {
      const arr = [
        ...
        { label: 'Quotation Templates', link: '/quotation_templates' },
        {
          ...
          link: '/quotation_templates/view/' + this.$route.params.id
        }
      ]
  ...
  mounted () {
    if (!this.$store.getters.hasPermissionTo('create_quotation')) {
      ...
      if (this.$route.params.id) {
        this.$q.loading.show()
        this.$axios
          .get('quotation_templates/' + this.$route.params.id)
          .then((res) => {
            this.model = res.data
  ...
  methods: {
    ...
    saveFn () {
      if (
        ...
        let route = 'quotation_templates'
        if (this.$route.params.id) {
          route = 'quotation_templates/' + this.$route.params.id
        }
        this.$q.loading.show()
        this.$axios
          .post(route, obj)
