<template>
    <q-page>
        <page-toolbar page-title="Product Types" :buttons="toolbarButtons" v-on:create="createFn"/>
        ...
        <q-dialog v-model="createDialog" persistent >
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div v-if="edit_mode">Edit product type: {{name}}</div>
                    <div v-else>Create new product type</div>
                    <q-space />
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        { label: 'Product Types', link: '/product_types' }
      ],
      pagePreferences: {
        page_index_visible_columns: 'product_types_index_visible_columns',
        page_index_pagination: 'product_types_index_pagination'
      }
    }
  },
  mounted () {
    this.$store.commit('setPageTitle', 'Product Types')
  },
  computed: {
    toolbarButtons () {
      const arr = []
  ...
  methods: {
    ...
    saveFn () {
      let route = 'product_types'
      if (this.edit_mode) {
        route = 'product_types/' + this.edit_id
      }
      const obj = {
