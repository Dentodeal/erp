<template>
    <q-page>
        <page-toolbar page-title="Product Categories" :buttons="toolbarButtons" v-on:create="createFn"/>
        ...
        <q-dialog v-model="createDialog" persistent >
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div v-if="edit_mode">Edit product category: {{name}}</div>
                    <div v-else>Create new product category</div>
                    <q-space />
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        { label: 'Product Departments', link: '/product_categories' }
      ],
      pagePreferences: {
        page_index_visible_columns: 'product_categories_index_visible_columns',
        page_index_pagination: 'product_categories_index_pagination'
      }
    }
  },
  mounted () {
    this.$store.commit('setPageTitle', 'Product Categories')
  },
  computed: {
    toolbarButtons () {
      const arr = []
  ...
  methods: {
    ...
    saveFn () {
      let route = 'product_categories'
      if (this.edit_mode) {
        route = 'product_categories/' + this.edit_id
      }
      const obj = {
