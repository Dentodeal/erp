<template>
    <q-page>
        <page-toolbar page-title="Product Departments" :buttons="toolbarButtons" v-on:create="createFn"/>
        <breadcrumbs :breadcrumbs="breadcrumbs"/>
         <simple-data-table
            route="product_departments"
        ...
        <q-dialog v-model="createDialog" persistent >
            <q-card style="width: 700px; max-width: 80vw;">
                <q-card-section class="row items-center q-pb-none">
                    <div v-if="edit_mode">Edit product department: {{name}}</div>
                    <div v-else>Create new product department</div>
                    <q-space />
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        { label: 'Product Departments', link: '/product_departments' }
      ],
      pagePreferences: {
        page_index_visible_columns: 'product_departments_index_visible_columns',
        page_index_pagination: 'product_departments_index_pagination'
      }
    }
  },
  mounted () {
    this.$store.commit('setPageTitle', 'Product Departments')
  },
  computed: {
    toolbarButtons () {
      const arr = []
  ...
  methods: {
    ...
    saveFn () {
      let route = 'product_departments'
      if (this.edit_mode) {
        route = 'product_departments/' + this.edit_id
      }
      const obj = {
